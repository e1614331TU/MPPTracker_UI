package comm;

/**
* PCDI_ParameterByte
* <p>
* this class implements the PCDI_Parameter for the type Long (4 Byte value)
* 
*/
public class PCDI_ParameterLong extends PCDI_Parameter<Integer> {
	/**
	 * the constructor needs all relevant information
	 * @param valNr is the value number of the parameter
	 * @param type is the value type number of the parameter
	 * @param value is the value of the parameter
	 */
	public PCDI_ParameterLong(int valNr, PCDI_TYPES type, Integer value) {
		super(valNr, type, value);
	}

	@Override
	public void setValue(String value) {
		super.setValue(Integer.parseInt(value));
		
	}

}
