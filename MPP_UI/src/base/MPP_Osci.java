...
public class MPP_Osci extends JPanel implements ActionListener, I_PCDI_Listener {
	...
	private void startTimerFast() {
		if(this.cyclicTimerFast==null) {
		    ...
		    this.cyclicTimerFast.scheduleAtFixedRate(task, 0, 500);
		}
	}
	...
	private void startTimerSlow() {
		if(this.cyclicTimerSlow==null) {
			TimerTask task = new TimerTask() {
		        public void run() {
		        	pcdi.getTableInfo(tableIdI, deviceId);
		        	pcdi.getTableInfo(tableIdP, deviceId);
		        	pcdi.getTableInfo(tableIdU, deviceId);
		        }
		    };
	...
	public void notifyTableRead(PCDI_TableData tableData, int deviceId) {
		...
		int minUequalzero =0;
		for(int i=tableData.getIndexNr();i>=0;i--) {
			if(this.data_U.size()>i && this.data_U.get(i)==0)
				minUequalzero=i;
		}
		...
		if(tableData.getTableId()==this.tableIdU) {
			...
			this.chart.updateXYSeries(this.nameScope2, this.data_U.subList(0, minUequalzero), this.data_P.subList(0, minUequalzero), null);
			//if(this.data_I.size()<minDataPoints)
				//minDataPoints=this.data_I.size();
			this.chart.updateXYSeries(this.nameScope1, this.data_U.subList(0, minUequalzero), this.data_I.subList(0, minUequalzero), null);
		}
